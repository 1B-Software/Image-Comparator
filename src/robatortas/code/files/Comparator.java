	...
	public void compares() {
		...
		try {
			...
			for(int i = 0; i < fileSize; i++) {
				...
				image = ImageIO.read(new File(dir + "\\" + filesInDir[i]));
				...
				image.getRGB(0, 0, ww, hh, comparedPixels, 0, ww);
				...
				// And I know I could just assigne image.getRGB() to a variable, but it's better this way for future use.
				
				for(int j = 0; j < pixels.length; i++) {
				...
				for(int j = 0; j < comparedPixels.length; i++) {
					int a = (comparedPixels[i] & 0xff000000) >> 24;
					int r = (comparedPixels[i] & 0xff0000) >> 16;
					int g = (comparedPixels[i] & 0xff00) >> 8;
					int b = (comparedPixels[i] & 0xff);
